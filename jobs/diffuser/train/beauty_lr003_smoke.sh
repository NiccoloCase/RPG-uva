...
# Submit from jobs/diffuser/train for a quick wiring check before full training.
#SBATCH --partition=gpu_a100
#SBATCH --job-name=drpg_beauty_lr003_smoke
#SBATCH --ntasks=1
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./beauty_lr003_smoke.sh" >&2
    exit 2
  fi
fi

export PRESET=beauty
export RUN_ID=drpg_beauty_lr003_smoke
export LR=0.003
export EPOCHS=1
