...
# Submit from jobs/diffuser/train for an H100 smoke run when gpu_h100 is freer.
#SBATCH --partition=gpu_h100
#SBATCH --job-name=drpg_beauty_lr003_smoke_h100
#SBATCH --ntasks=1
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./beauty_lr003_smoke_h100.sh" >&2
    exit 2
  fi
fi

export PRESET=beauty
export RUN_ID=drpg_beauty_lr003_smoke_h100
export LR=0.003
export EPOCHS=1
