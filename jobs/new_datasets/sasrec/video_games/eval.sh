...
#SBATCH --job-name=sasrec_video_games_eval
#SBATCH --ntasks=1
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_ROOT}/artifacts/sasrec/ckpt/sasrec_video_games.pt}"

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "sasrec_video_games_eval"
...
runtime_stats_run python3 scripts/sasrec.py --preset video_games --dataset Video_Games --eval-only --checkpoint "${CHECKPOINT_PATH}"
