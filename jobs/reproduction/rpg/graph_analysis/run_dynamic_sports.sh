...
#SBATCH --job-name=rpg_graph_dynamic
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_dynamic_sports.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
DYNAMIC_CONFIG="${DYNAMIC_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/sports_dynamic.yaml}"
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_dynamic"
