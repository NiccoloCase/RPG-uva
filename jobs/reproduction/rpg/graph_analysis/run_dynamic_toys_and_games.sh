...
#SBATCH --job-name=rpg_graph_dynamic_toys
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_dynamic_toys_and_games.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
DYNAMIC_CONFIG="${DYNAMIC_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/toys_and_games_dynamic.yaml}"
...
if [[ ! -f "${DYNAMIC_CONFIG}" ]]; then
  echo "ERROR: dynamic graph analysis config not found: ${DYNAMIC_CONFIG}" >&2
  exit 7
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_dynamic_toys"
