...
#SBATCH --job-name=rpg_graph_frontier_memory
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_frontier_memory_sports.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
FRONTIER_CONFIG="${FRONTIER_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/sports_frontier_memory.yaml}"
...
if [[ ! -f "${FRONTIER_CONFIG}" ]]; then
  echo "ERROR: frontier-memory graph analysis config not found: ${FRONTIER_CONFIG}" >&2
  exit 7
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_frontier_memory"
