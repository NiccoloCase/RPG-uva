...
#SBATCH --job-name=rpg_graph_novelty
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_novelty_sports.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
NOVELTY_CONFIG="${NOVELTY_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/sports_novelty.yaml}"
...
if [[ ! -f "${NOVELTY_CONFIG}" ]]; then
  echo "ERROR: novelty graph analysis config not found: ${NOVELTY_CONFIG}" >&2
  exit 7
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_novelty"
