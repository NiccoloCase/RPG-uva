...
# Requires an existing Beauty graph-analysis session with graphs/graph_metadata.json.
# If SESSION_DIR is omitted, the latest prepared Beauty graph-analysis session is used.
# This job measures no-trace inference latency, not Experiment C tracing latency.
#SBATCH --job-name=rpg_graph_perf_beauty
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_perf_inference_beauty.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
PERF_CONFIG="${PERF_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/beauty_perf_inference.yaml}"
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_perf_beauty"
