...
#SBATCH --job-name=rpg_graph_perf_cds
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_perf_inference_cds_and_vinyl.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
PERF_CONFIG="${PERF_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/cds_and_vinyl_perf_inference.yaml}"
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_perf_cds"
