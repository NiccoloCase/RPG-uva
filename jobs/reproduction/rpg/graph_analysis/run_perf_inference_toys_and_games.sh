...
# Requires an existing Toys graph-analysis session with graphs/graph_metadata.json.
# If SESSION_DIR is omitted, the latest prepared Toys graph-analysis session is used.
# This job measures no-trace inference latency, not Experiment C tracing latency.
#SBATCH --job-name=rpg_graph_perf_toys
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_perf_inference_toys_and_games.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
GRAPH_CONFIG="${GRAPH_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/toys_and_games.yaml}"
PERF_CONFIG="${PERF_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/toys_and_games_perf_inference.yaml}"
...
if [[ -z "${CHECKPOINT_PATH}" ]]; then
  echo "ERROR: provide the Toys RPG checkpoint path as the first argument or CHECKPOINT_PATH env var." >&2
  exit 3
fi
...
if [[ ! -f "${PERF_CONFIG}" ]]; then
  echo "ERROR: inference performance config not found: ${PERF_CONFIG}" >&2
  exit 7
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_perf_toys"
