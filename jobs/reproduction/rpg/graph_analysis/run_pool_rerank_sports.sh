...
#SBATCH --job-name=rpg_graph_pool
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_pool_rerank_sports.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
POOL_CONFIG="${POOL_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/sports_pool_rerank.yaml}"
...
if [[ ! -f "${POOL_CONFIG}" ]]; then
  echo "ERROR: visited-pool graph analysis config not found: ${POOL_CONFIG}" >&2
  exit 7
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_pool"
