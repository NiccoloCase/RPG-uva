...
# Example:
#   cd /gpfs/home6/$USER/RPG/jobs/reproduction/rpg/graph_analysis
#   sbatch ./run_rerank_sports.sh /projects/prjs2120/groups/group_16/artifacts/rpg/ckpt/model.pth
#SBATCH --job-name=rpg_graph_rerank
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_rerank_sports.sh /projects/prjs2120/groups/group_16/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
RERANK_CONFIG="${RERANK_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/sports_rerank.yaml}"
...
if [[ ! -f "${RERANK_CONFIG}" ]]; then
  echo "ERROR: rerank graph analysis config not found: ${RERANK_CONFIG}" >&2
  exit 7
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_rerank"
