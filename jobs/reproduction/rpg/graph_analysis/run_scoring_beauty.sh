...
#SBATCH --job-name=rpg_graph_scoring_beauty
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_scoring_beauty.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
SCORING_CONFIG="${SCORING_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/beauty_scoring.yaml}"
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_scoring_beauty"
