...
# Requires an existing CDs graph-analysis session with graphs/graph_metadata.json.
# If SESSION_DIR is omitted, the latest prepared CDs graph-analysis session is used.
#SBATCH --job-name=rpg_graph_scoring_cds
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_scoring_cds_and_vinyl.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
GRAPH_CONFIG="${GRAPH_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/cds_and_vinyl.yaml}"
SCORING_CONFIG="${SCORING_CONFIG:-${REPO_ROOT}/configs/rpg/graph_analysis/cds_and_vinyl_scoring.yaml}"
...
if [[ -z "${CHECKPOINT_PATH}" ]]; then
  echo "ERROR: provide the CDs RPG checkpoint path as the first argument or CHECKPOINT_PATH env var." >&2
  exit 3
fi
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_scoring_cds"
