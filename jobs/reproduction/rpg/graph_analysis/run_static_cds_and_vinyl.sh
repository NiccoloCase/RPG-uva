...
# This builds the exact flat top-500 CDs graph, then runs static graph analysis.
#SBATCH --job-name=rpg_graph_static_cds
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_static_cds_and_vinyl.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_static_cds"
...
SESSION_DIR="${REPO_ROOT}/artifacts/rpg/graph_analysis/cds_and_vinyl/${SESSION_NAME}"
