...
#SBATCH --job-name=rpg_graph_static
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_static_sports.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_static"
...
SESSION_DIR="${REPO_ROOT}/artifacts/rpg/graph_analysis/sports/${SESSION_NAME}"
