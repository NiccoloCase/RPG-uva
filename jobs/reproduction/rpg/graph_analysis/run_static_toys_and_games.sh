...
# This builds the exact flat top-100 Toys graph, then runs static graph analysis.
#SBATCH --job-name=rpg_graph_static_toys
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./run_static_toys_and_games.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_graph_static_toys"
...
SESSION_DIR="${SESSION_DIR:-${REPO_ROOT}/artifacts/rpg/graph_analysis/toys_and_games/${SESSION_NAME}}"
