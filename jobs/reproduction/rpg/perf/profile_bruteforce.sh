...
# This profiles exact no-graph RPG scoring over enlarged Sports candidate pools.
#SBATCH --job-name=rpg_perf_bruteforce
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./profile_bruteforce.sh /gpfs/home6/\$USER/RPG/artifacts/rpg/ckpt/model.pth" >&2
    exit 2
  fi
fi
...
if [[ -n "${ITEM_CHUNK_SIZE}" ]]; then
  extra_args+=(--item-chunk-size "${ITEM_CHUNK_SIZE}")
fi
