...
# Submit from jobs/reproduction/rpg/toys_and_games so these relative output paths resolve correctly.
...
#SBATCH --job-name=rpg_toys_eval
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=18
#SBATCH --mem=120G
#SBATCH --time=02:00:00
#SBATCH --gpus=1
#SBATCH --output=../../../../output/reproduction/rpg/toys_and_games/%x-%j.out
#SBATCH --error=../../../../output/reproduction/rpg/toys_and_games/%x-%j.err
...
OUTPUT_DIR="${REPO_ROOT}/output/reproduction/rpg/toys_and_games"
DATA_DIR="${REPO_ROOT}/artifacts/rpg/cache/AmazonReviews2014/Toys_and_Games"
...
runtime_stats_init "${OUTPUT_DIR}" "rpg_toys_eval"
...
if [[ -z "${CHECKPOINT_PATH}" ]]; then
  CHECKPOINT_PATH="$(
    find "${CKPT_DIR}" -maxdepth 1 -type f -name 'rpg_repro_toys_and_games-*.pth' -printf '%T@ %p\n' \
      | sort -nr \
      | sed -n '1s/^[^ ]* //p'
  )"
fi
...
if [[ -z "${CHECKPOINT_PATH}" || ! -f "${CHECKPOINT_PATH}" ]]; then
  echo "ERROR: could not find a Toys_and_Games checkpoint in ${CKPT_DIR}" >&2
