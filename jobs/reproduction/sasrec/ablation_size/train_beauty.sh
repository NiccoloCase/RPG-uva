...
#SBATCH --job-name=sasrec_beauty_size_train
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=18
#SBATCH --mem=120G
#SBATCH --time=48:00:00
#SBATCH --gpus=1
#SBATCH --output=../../../../output/reproduction/sasrec/ablation_size/%x-%j.out
#SBATCH --error=../../../../output/reproduction/sasrec/ablation_size/%x-%j.err
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
    ...
    echo "  bash ./train_beauty.sh" >&2
    exit 2
  fi
fi

REPO_ROOT="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
OUTPUT_DIR="${REPO_ROOT}/output/reproduction/sasrec/ablation_size"
...
runtime_stats_init "${OUTPUT_DIR}" "sasrec_beauty_size_train"
...
runtime_stats_run python3 scripts/sasrec.py \
  ...
  --run_id sasrec_beauty_size_match \
  --hidden_size 540
