...
OUTPUT_DIR="${REPO_ROOT}/output/reproduction/sasrec/beauty"
DATA_FILE="${REPO_ROOT}/artifacts/sasrec/data/Beauty/Beauty.txt"
ENV_PREFIX="${REPO_ROOT}/artifacts/conda/rpg-uva"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_ROOT}/artifacts/sasrec/ckpt/sasrec_beauty.pt}"

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "sasrec_beauty_eval"
...
runtime_stats_run python3 scripts/sasrec.py --preset beauty --dataset Beauty --eval-only --checkpoint "${CHECKPOINT_PATH}"
