...
#SBATCH --array=0-3
#SBATCH --output=../../../../output/reproduction/sasrec/perf/%x-%A_%a.out
#SBATCH --error=../../../../output/reproduction/sasrec/perf/%x-%A_%a.err
...
LOCAL_BASELINE_DIR="${REPO_ROOT}/artifacts/sasrec/ckpt"
SHARED_BASELINE_DIR="/projects/prjs2120/groups/group_16/artifacts/sasrec/ckpt"
...
BASELINE_CHECKPOINT="${LOCAL_BASELINE_DIR}/sasrec_${DATASET_SLUG}.pt"
if [[ ! -f "${BASELINE_CHECKPOINT}" ]]; then
  BASELINE_CHECKPOINT="${SHARED_BASELINE_DIR}/sasrec_${DATASET_SLUG}.pt"
fi
if [[ ! -f "${BASELINE_CHECKPOINT}" ]]; then
...
runtime_stats_init "${OUTPUT_DIR}" "sasrec_perf_compare_${DATASET_SLUG}"
...
BASE_PROFILE_ROOT="artifacts/sasrec/perf/${DATASET_SLUG}/baseline"
BEST_PROFILE_ROOT="artifacts/sasrec/perf/${DATASET_SLUG}/best_ablation"
...
COMPARE_OUTPUT_DIR="${REPO_ROOT}/artifacts/sasrec/perf/${DATASET_SLUG}/comparison/$(date -u +%Y%m%dT%H%M%SZ)_job${SLURM_JOB_ID:-manual}"
