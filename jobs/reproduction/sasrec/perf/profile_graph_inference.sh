...
#SBATCH --output=../../../../output/reproduction/sasrec/perf/graph_profile/%x-%j.out
#SBATCH --error=../../../../output/reproduction/sasrec/perf/graph_profile/%x-%j.err
...
OUTPUT_DIR="${REPO_ROOT}/output/reproduction/sasrec/perf/graph_profile"
...
if [[ ! -f "${PERF_CONFIG}" ]]; then
  echo "ERROR: SASRec graph perf config not found: ${PERF_CONFIG}" >&2
  exit 5
fi
...
runtime_stats_init "${OUTPUT_DIR}" "sasrec_graph_profile_${PERF_DATASET_SLUG}"
