...
#SBATCH --job-name=sasrec_sports_eval
#SBATCH --ntasks=1
...
OUTPUT_DIR="${REPO_ROOT}/output/reproduction/sasrec/sports_and_outdoors"
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_ROOT}/artifacts/sasrec/ckpt/sasrec_sports_and_outdoors.pt}"

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "sasrec_sports_and_outdoors_eval"
...
runtime_stats_run python3 scripts/sasrec.py --preset sports_and_outdoors --dataset Sports_and_Outdoors --eval-only --checkpoint "${CHECKPOINT_PATH}"
