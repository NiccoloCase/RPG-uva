...
#SBATCH --job-name=sasrec_sports_and_outdoors_eval
#SBATCH --ntasks=1
...
init_sasrec_job "${OUTPUT_DIR}" "sasrec_sports_and_outdoors_eval"
load_sasrec_env

CKPT_DIR="${REPO_ROOT}/artifacts/sasrec/ckpt"
CHECKPOINT_PATH="$(resolve_checkpoint_path "${1:-}" "${CKPT_DIR}" "sasrec_repro_sports_and_outdoors")"
EVAL_SEED="${EVAL_SEED:-2024}"

cd "${REPO_ROOT}"

echo "Evaluating SASRec Sports_and_Outdoors checkpoint: ${CHECKPOINT_PATH}"
