...
#SBATCH --job-name=sasrec_toys_eval
#SBATCH --ntasks=1
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_ROOT}/artifacts/sasrec/ckpt/sasrec_toys_and_games.pt}"

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "sasrec_toys_and_games_eval"
...
runtime_stats_run python3 scripts/sasrec.py --preset toys_and_games --dataset Toys_and_Games --eval-only --checkpoint "${CHECKPOINT_PATH}"
