...
mkdir -p \
  ...
  "${REPO_ROOT}/output/reproduction/sasrec_modernized/ablation_size/lr_depth_grid"

cd "${REPO_ROOT}/jobs/init/env"
install_job_id="$(sbatch --parsable "${account_args[@]}" ./setup_env.sh)"

declare -A TRAIN_SCRIPTS=(
  [sports_and_outdoors]="train_sports_lr_depth_grid.sh"
  [beauty]="train_beauty_lr_depth_grid.sh"
  [toys_and_games]="train_toys_lr_depth_grid.sh"
  [cds_and_vinyl]="train_cds_lr_depth_grid.sh"
)
...
printf 'Submitted SASRec modernized size-match LR-depth-grid jobs\n'
printf 'install=%s\n' "${install_job_id}"
