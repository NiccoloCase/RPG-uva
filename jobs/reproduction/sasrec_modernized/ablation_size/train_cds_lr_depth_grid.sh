...
#SBATCH --job-name=sasrec_mod_cds_lr_depth
#SBATCH --ntasks=1
...
if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
  ...
  echo "  sbatch ./train_cds_lr_depth_grid.sh" >&2
  exit 2
fi
...
LR_TAG="${LR//./p}"
RUN_ID="sasrec_modernized_cds_and_vinyl_size_match_e300_lr${LR_TAG}_L${NUM_HIDDEN_LAYERS}"
