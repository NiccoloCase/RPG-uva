...
#SBATCH --job-name=sasrec_mod_sports_lr_depth
#SBATCH --ntasks=1
...
if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
  ...
  echo "  sbatch ./train_sports_lr_depth_grid.sh" >&2
  exit 2
fi
...
LR_TAG="${LR//./p}"
RUN_ID="sasrec_modernized_sports_and_outdoors_size_match_e300_lr${LR_TAG}_L${NUM_HIDDEN_LAYERS}"
