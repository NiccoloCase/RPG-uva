...
#SBATCH --job-name=sasrec_modernized_cds_eval
#SBATCH --ntasks=1
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_ROOT}/artifacts/sasrec_modernized/ckpt/sasrec_modernized_cds_and_vinyl.pt}"

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "sasrec_modernized_cds_and_vinyl_eval"
...
runtime_stats_run python3 scripts/sasrec_modernized.py --preset cds_and_vinyl --dataset CDs_and_Vinyl --eval-only --checkpoint "${CHECKPOINT_PATH}"
