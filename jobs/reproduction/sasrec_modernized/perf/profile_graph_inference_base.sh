...
# SASRecModernized base graph-profile run on Sports using the best graph grid setting.
# Submit from this directory:
#   cd jobs/reproduction/sasrec_modernized/perf
#   mkdir -p ../../../../output/reproduction/sasrec_modernized/perf/graph_profile_base
...
#SBATCH --output=../../../../output/reproduction/sasrec_modernized/perf/graph_profile_base/%x-%j.out
#SBATCH --error=../../../../output/reproduction/sasrec_modernized/perf/graph_profile_base/%x-%j.err
...
OUTPUT_DIR="${REPO_ROOT}/output/reproduction/sasrec_modernized/perf/graph_profile_base"
ENV_PREFIX="${REPO_ROOT}/artifacts/conda/rpg-uva"
PERF_CONFIG="${REPO_ROOT}/configs/sasrec/perf/sports_graph.yaml"
PERF_OUTPUT_DIR="${REPO_ROOT}/artifacts/sasrec_modernized/perf/graph/base/sports_and_outdoors"
...
if [[ ! -f "${CHECKPOINT_PATH}" ]]; then
  echo "ERROR: SASRecModernized base checkpoint not found: ${CHECKPOINT_PATH}" >&2
  exit 4
fi

mkdir -p "${OUTPUT_DIR}"
source "${REPO_ROOT}/jobs/lib/runtime_stats.sh"
runtime_stats_init "${OUTPUT_DIR}" "sasrec_modernized_graph_profile_base_${PERF_DATASET_SLUG}"
