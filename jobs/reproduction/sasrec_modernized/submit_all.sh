...
TARGET_DIR="${REPO_ROOT}/jobs/reproduction/sasrec_modernized/${DATASET}"

if [[ ! -d "${TARGET_DIR}" ]]; then
  echo "ERROR: unknown SASRec modernized dataset directory: ${TARGET_DIR}" >&2
  exit 2
fi
...
mkdir -p \
  ...
  "${REPO_ROOT}/output/reproduction/sasrec_modernized/${DATASET}"
...
printf 'Submitted SASRec modernized jobs for DATASET=%s\n' "${DATASET}"
printf 'install=%s\n' "${install_job_id}"
printf 'prepare=%s\n' "${prepare_job_id}"
printf 'train=%s\n' "${train_job_id}"
printf 'eval=%s\n' "${eval_job_id}"
