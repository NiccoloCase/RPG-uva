...
#SBATCH --job-name=rpg_sweep_m_cds
...
if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
  ...
  echo "  sbatch ./cds_and_vinyl.sh" >&2
  exit 2
fi
...
RUN_ID="rpg_sweep_m${M}_cds_and_vinyl"

echo "SWEEP_START dataset=CDs_and_Vinyl n_codebook=${M} run_id=${RUN_ID}"
...
echo "SWEEP_END dataset=CDs_and_Vinyl n_codebook=${M} run_id=${RUN_ID}"
