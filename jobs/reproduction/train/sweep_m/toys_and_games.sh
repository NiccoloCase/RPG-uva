...
#SBATCH --job-name=rpg_sweep_m_toys
...
if [[ "${PWD_REAL}" != "${SCRIPT_DIR}" ]]; then
  ...
  echo "  sbatch ./toys_and_games.sh" >&2
  exit 2
fi
...
RUN_ID="rpg_sweep_m${M}_toys_and_games"

echo "SWEEP_START dataset=Toys_and_Games n_codebook=${M} run_id=${RUN_ID}"
...
echo "SWEEP_END dataset=Toys_and_Games n_codebook=${M} run_id=${RUN_ID}"
