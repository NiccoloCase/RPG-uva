...
from models.sasrec_modernized import (  # noqa: E402
    SASRecModernizedDataset,
    SASRecModernizedModel,
    SASRecModernizedTrainer,
)
from models.sasrec_modernized.utils import EarlyStopping, check_path, get_user_seqs, set_seed  # noqa: E402

MODEL_CONFIG = REPO_ROOT / "models" / "sasrec_modernized" / "config.yaml"
ROOT_CONFIG = REPO_ROOT / "configs" / "sasrec_modernized" / "root.yaml"
LOCAL_CONFIG = REPO_ROOT / "configs" / "sasrec_modernized" / "local.yaml"
PRESET_CONFIGS = {
    "beauty": REPO_ROOT / "configs" / "sasrec_modernized" / "beauty.yaml",
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="Run the modernized standalone SASRec reproduction path from the repository root.",
        epilog=(
            "Config precedence: CLI overrides > --config files > --preset file > "
            "configs/sasrec_modernized/local.yaml > configs/sasrec_modernized/root.yaml > "
            "models/sasrec_modernized/config.yaml."
        ),
    )
    ...
    parser.add_argument("--no-root-config", action="store_true", help="Skip configs/sasrec_modernized/root.yaml.")
    parser.add_argument("--no-local-config", action="store_true", help="Skip configs/sasrec_modernized/local.yaml.")
...
def normalize_config(config: dict[str, object], checkpoint_override: str | None) -> SimpleNamespace:
    ...
    normalized["log_dir"] = str(resolve_repo_path(normalized.get("log_dir", "artifacts/sasrec_modernized/logs")))
    normalized["ckpt_dir"] = str(resolve_repo_path(normalized.get("ckpt_dir", "artifacts/sasrec_modernized/ckpt")))
    normalized["output_dir"] = str(
        resolve_repo_path(normalized.get("output_dir", "output/reproduction/sasrec_modernized"))
    )

    data_name = str(normalized["data_name"])
    run_id = str(normalized.get("run_id", f"sasrec_modernized_{data_name.lower()}"))
...
def build_dataloaders(args: SimpleNamespace, user_seq: list[list[int]]):
    train_dataset = SASRecModernizedDataset(args, user_seq, data_type="train")
    valid_dataset = SASRecModernizedDataset(args, user_seq, data_type="valid")
...
def main() -> int:
    ...
    model = SASRecModernizedModel(args)
    trainer = SASRecModernizedTrainer(model, train_dataloader, valid_dataloader, test_dataloader, args)
